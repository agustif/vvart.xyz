  ...
  return (
    <div
      className={cn("text-md my-6 flex items-start rounded-lg border-2 border-l-8 bg-gray-100 p-4 font-normal text-gray-700", {
        ...
        "border-black bg-gray-50 dark:bg-gray-950 dark:border-gray-800": type === "info",
        // type info blueish
      })}
      {...props}
    >
      {icon && <span className="mr-4">{<Icons.info size={36} className="fill-black dark:fill-white" />}</span>}
      <div>{children}</div>
    </div>
  )
}
